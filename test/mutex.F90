program test_simple_mutex
    ...
    type(App_Mutex), dimension(:), allocatable :: mutexes
    ...
    if (num_pes < 2) then
        call app_log(APP_ERROR, 'We need at least 2 processes for this test')
        error stop 1
    end if

    shared_mem      = app_allocate_shared(int(num_pes * 4, kind=8), MPI_COMM_WORLD)
    shmem_variables = app_allocate_shared(int(num_pes * 4 * 3, kind=8), MPI_COMM_WORLD)

    if (.not. C_ASSOCIATED(shared_mem) .or. .not. C_ASSOCIATED(shmem_variables)) then
        write(app_msg, *) 'Could not allocated shared memory for the mutexes'
    ...
    if (test_array(rank + 1, 1) .ne. NUM_IT * num_pes) then
        write(app_msg, *) 'Wrong final value for incremented value. Correct vs actual: ', NUM_IT * num_pes, test_array(rank + 1, 1)
        call app_log(APP_ERROR, app_msg)
        error stop 1
    end if

    if (test_array(rank + 1, 2) + test_array(rank + 1, 3) .ne. NUM_IT * num_pes) then
        write(app_msg, *) 'Wrong final value for try_lock test.', test_array(rank + 1, 2), test_array(rank + 1, 3)
        call app_log(APP_ERROR, app_msg)
        error stop 1
    end if

    if (num_errors > 0) then
        write(app_msg, *) 'num_errors is non-zero', num_errors
        call app_log(APP_ERROR, app_msg)
        error stop 1
    end if
    ...
    if (rank == 0) then
        call App_Log(APP_INFO, 'Simple mutex test successful')
        print *, test_array(:, 2)
        print *, test_array(:, 3)
    end if
